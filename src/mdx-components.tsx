...
const components = {
	...
	p: ({ children }) => <p className="leading-5 not-first:mt-6">{children}</p>,
	ul: ({ children }) => (
		<ul className="my-6 ml-6 [&>li>ol]:my-2 [&>li>ul]:my-2 list-disc [&>li]:mt-1 not-first:mt-1">
			{children}
		</ul>
	),
	ol: ({ children }) => (
		<ol className="my-6 ml-6 [&>li>ol]:my-2 [&>li>ul]:my-2 list-decimal [&>li]:mt-1 not-first:mt-1">
			{children}
		</ol>
	),
	a: ({ children, ...props }) => (
